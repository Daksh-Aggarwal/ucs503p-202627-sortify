...
# W4: Model Decision and Intial Planning for prototype

## Objective

My this week's objective was to help my team with the intial design and implementation of our 
app and simultaneously finalise on the model design we will use for the prototype of sortify.

## Work Done

1. Helped team to come up for intial design ie. icon and layout for our app before starting implementation.
2. Finalized on using a pretrained model with training and hypertuning on [phenomsg's waste dataset.]
(https://www.kaggle.com/datasets/phenomsg/waste-classification) for next prototype testing.
3. Began to prepare this dataset for preprocessing and cleaning to check for missing labels or wrong
images in labels.

## Outcomes
- Intial design for our app got handled for now.
- Dataset is decided with 5 major classes (Recycleable,Organic,General,E-waste,Hazadous) covering our intial
plan for the protype and model training  
- Preprocessing phase is completed making us comfortable for model training.

## My Learning
- Merging the medical dataset with phenomsg's was too tedious of job because of the different classes so deciding on 
later for now is the best option for the prototype.


# W5: Finalising Prototype and Making ER and Activity Diagram

## Objective

This week's goal was to finalize the ER and activity diagrams for the proposal's design section and support the team in wrapping up the prototype-stage report.

## Work Done

I built the ER diagram covering User, Scan, Feedback, Waste-Item, Waste-Category, and Location, marking Scan and Feedback as weak entities tied to their owning entity through identifying relationships (Performs, Receives), and modeling the many-to-many link between Waste-Item and Location through the Applies-For relationship, which carries the disposal method and precautionary instructions. I also drew the activity diagram for the core scan to guidance flow, including the low-confidence loop back to capture/upload and the manual-search fallback, and the final branch between saving a scan and submitting feedback. Alongside this, I helped review the prototype report to make sure the diagrams matched what was actually implemented since the prototype itself is a frontend-only demo with simulated ML predictions and local storage, not a connected backend.

## Outcomes

Both diagrams were finalized and included in the design section, and the prototype report accurately reflects the current implementation status rather than overstating progress.

## My Learnings
Modeling Scan and Feedback as weak entities forced me to think about what actually gives them identity — a scan only means something tied to a user, and that dependency is worth showing explicitly in the diagram, not just assumed.