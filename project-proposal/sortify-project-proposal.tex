...
\begin{itemize}[leftmargin=0.7cm]
    ...
    \item \textbf{Conversational assistant (chatbot):} an integrated chatbot lets users ask natural-language questions about waste disposal, clarify ambiguous items, and get guidance without manually searching the knowledge base.
\end{itemize}

\subsection{System Context Diagram}

Figure~\ref{fig:level0} provides a high-level view of Sortify and its interaction with the primary external actors.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.68\textwidth]{level0.png}
    \caption{Level-0 Context Diagram for Sortify}
    \label{fig:level0}
\end{figure}
...
Figure~\ref{fig:use-case-diagram} illustrates the major functional interactions between Sortify users, administrators, and the system.
...
Figure~\ref{fig:level1} expands the system context into the main functional processes and data interactions within Sortify.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{level1.png}
...
Figure~\ref{fig:ml-process} summarizes the main processing stages of the classification component.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{3.0model.png}
    \caption{ML Classification Process}
...
\begin{itemize}[leftmargin=0.7cm]
    ...
    \item Chatbot interface for natural-language disposal queries.
\end{itemize}
...
\begin{itemize}[leftmargin=0.7cm]
    ...
    \item Chatbot query handling and conversation storage.
\end{itemize}
...
Figure~\ref{fig:user-data-flow} illustrates the main user-facing data operations involving saved scans, feedback, and scan history.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{3.0userdata.png}
    \caption{User Data and Feedback Flow}
    \label{fig:user-data-flow}
\end{figure}

Figure~\ref{fig:admin-flow} illustrates the corresponding administrative operations for managing the catalog and reviewing feedback and analytics.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.62\textwidth]{3.0admin.png}
    \caption{Administrative Management Flow}
...
Figure~\ref{fig:map-guide} shows how waste information and location context contribute to disposal guidance.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{3.0map.png}
    \caption{Location-Aware Disposal Guidance Flow}
    \label{fig:map-guide}
\end{figure}
...
\begin{itemize}[leftmargin=0.7cm]
    \item \textbf{Grounded responses:} the chatbot will retrieve relevant entries from the waste knowledge base (retrieval-augmented generation) rather than relying on an unconstrained language model, reducing the risk of incorrect disposal advice.
    \item \textbf{Fallback and escalation:} when the assistant cannot confidently answer a query, it will direct the user to manual search or suggest capturing an image for ML-based identification.
    \item \textbf{Integration with scan results:} after a scan, users can ask follow-up questions about the identified item directly within the chatbot interface.
    \item \textbf{Conversation history:} authenticated users' chatbot conversations can be stored alongside scan history for continuity across sessions.
    \item \textbf{Scope control:} the assistant will be restricted to waste-management and disposal-related topics to keep responses relevant and reliable.
\end{itemize}

Figure~\ref{fig:chatbot-flow} summarizes the chatbot's query and response flow.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{3.0chatbot.drawio.png}
    \caption{Conversational Assistant Flow}
    \label{fig:chatbot-flow}
\end{figure}
...
\begin{itemize}[leftmargin=0.7cm]
    ...
    \item \textbf{Conversational AI:} an LLM API combined with retrieval-augmented generation over the waste knowledge base to power the chatbot.
...
\begin{itemize}[leftmargin=0.7cm]
    ...
    \item Conversational assistant (chatbot) for natural-language disposal queries.
...
\begin{itemize}[leftmargin=0.7cm]
    ...
    \item \textbf{Chatbot reliability:} ground chatbot responses in the waste knowledge base via retrieval-augmented generation and restrict its scope to avoid hallucinated or unsafe disposal advice.
\end{itemize}
